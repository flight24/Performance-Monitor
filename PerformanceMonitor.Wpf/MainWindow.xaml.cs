...
public partial class MainWindow : Window
{
    ...
    private static readonly IntPtr HwndBottom = new IntPtr(1);   // HWND_BOTTOM
    ...
    /// <summary>裁剪工作集：把冷页换出物理内存，显著降低任务管理器中的占用。</summary>
    ...
    private void OnLoaded(object sender, RoutedEventArgs e)
    {
        ...
        // 拦截 WM_MOUSEACTIVATE：取消置顶时点击不抬起窗口，始终保持底层
        ...
        periodicTrim.Tick += (_, _) => TrimWorkingSet();
        periodicTrim.Start();
    }
    ...
    private IntPtr WndProcHook(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
    {
        if (!_pinEnabled)
        {
            if (msg == WmWindowPosChanging)
            {
                ...
                wp.hwndInsertAfter = HwndBottom;
                wp.flags |= SwpNoActivate;
