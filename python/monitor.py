...
class _SilentPopen:
    ...
    def __enter__(self):
        return self
    def __exit__(self, *a):
        if self._popen.stdout: self._popen.stdout.close()
        if self._popen.stderr: self._popen.stderr.close()
        self._popen.__exit__(*a)
...
def get_cpu_freq():
    if _cpu_perf_handle and _cpu_base_freq:
        try:
            ...
            dt = ctypes.c_ulong()
            v = ctypes.c_double()
            rc = pdh.PdhGetFormattedCounterValue(_cpu_perf_handle, 0x00000200, ctypes.byref(dt), ctypes.byref(v))
            if rc == 0 and v.value and v.value > 0:
                return (v.value / 100.0) * _cpu_base_freq / 1000.0
        except Exception:
            pass
    # Fallback: LHM clock sensors (max core clock)
    if _lhm_cpu:
        try:
            _lhm_cpu.Update()
            best = 0.0
            for s in _lhm_cpu.Sensors:
                if str(s.SensorType) == "Clock" and s.Value is not None and s.Name != "Bus Speed":
                    v = float(s.Value)
                    if v > best:
                        best = v
            if best > 0:
