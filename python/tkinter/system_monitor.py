...
def _get_lhm_sensor_by_type(hw, sensor_type):
    if hw is None:
        return None
    hw.Update()
    for sensor in hw.Sensors:
        if str(sensor.SensorType) == sensor_type and sensor.Value is not None:
            return float(sensor.Value)
    return None
...
def get_gpu_temp_lhm():
    ...
    if _lhm_gpu is None:
        return None
    try:
        return _get_lhm_sensor(_lhm_gpu, [
            "GPU Core",
            "GPU Package",
            "GPU Hotspot",
        ])
...
def get_gpu_load_lhm():
    _init_lhm()
    if _lhm_gpu is None:
        return None
    try:
        load = _get_lhm_sensor(_lhm_gpu, "GPU Core")
        if load is not None:
            return load
        load = _get_lhm_sensor_by_type(_lhm_gpu, "Load")
        if load is not None:
            return load
        load = _get_lhm_sensor(_lhm_gpu, "D3D 3D")
        if load is not None:
            return load
        load = _get_lhm_sensor(_lhm_gpu, "D3D Usage")
        if load is not None:
            return load
        load = _get_lhm_sensor_by_type(_lhm_gpu, "SmallData")
        if load is not None:
            return load
    except Exception:
        pass
    ...
    def _loop(self):
        ...
        while self._running:
            try:
                ...
                if gpu_load == 0:
                    try:
                        lhm_load = get_gpu_load_lhm()
                        if lhm_load is not None:
                            gpu_load = lhm_load
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        if c:
                            for e in c.Win32_PerfFormattedData_GPUPerformanceCounters_GPUEngine():
                                name = str(e.Name)
                                if "3D" in name:
                                    val = float(e.PercentOfTimeInUse) if e.PercentOfTimeInUse else 0
                                    if val > gpu_load:
                                        gpu_load = val
                            for a in c.Win32_PerfFormattedData_GPUPerformanceCounters_GPUAdapter():
                                val = float(a.PercentOfTimeInUse) if a.PercentOfTimeInUse else 0
                                if val > gpu_load:
                                    gpu_load = val
                    except Exception:
                        pass
